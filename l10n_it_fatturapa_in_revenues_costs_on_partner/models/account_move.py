...
class AccountMovue(models.Model):
    ...
    def create(self, vals_list):
        for val in vals_list:
            ...
            move_lines_product = [item for item in val['invoice_line_ids'] if item[2].get('display_type') == 'product']
            account_id = False
            partner_id = self.env['res.partner'].browse(int(val['partner_id']))
            if res['move_type'] in ['out_invoice', 'out_refund'] and partner_id.revenues_account_id:
                account_id =  partner_id.revenues_account_id.id
            elif res['move_type'] in ['in_invoice', 'in_refund'] and partner_id.costs_account_id:
                account_id =  partner_id.costs_account_id.id
            if account_id:
                move_lines_product[0][2]['account_id'] = account_id
