...
class Res_partner_costs_revenues_account_id_rel(models.Model):
    ...
    def _compute_domain_account_id(self):
        for record in self:
            record.domain_account_id = "[('account_type', 'in', ['expense', 'asset_receivable','asset_cash','asset_current','asset_non_current','asset_fixed','asset_prepayments',  'expense','expense_direct_cost','expense_depreciation','equity'])]" if record.type == 'costs' else \
